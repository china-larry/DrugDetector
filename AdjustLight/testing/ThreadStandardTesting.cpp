...
//        qDebug() << "GetVideoCapture:" << strPath;
        if(m_QRCodeInfo.eTypeCup == TypeTCup)       //圆杯
        {
            ...
            else if(m_iIndexMovement < COUNT_PIC_LOCATION)
            {
                bool bExist;
                _ReceivePicPathTCup(strPath, bExist,true);
                ++m_iIndexMovement;
            }
            else                            //定位条之后项目，发送测试结果
            {
 ...
 * @return 返回测试结果
 */
TestResultData ThreadStandardTesting::_ReceivePicPathTCup(QString strPath, bool &bExist,bool bFixSept)
{
...
void ThreadStandardTesting::test()
{
...
int ThreadStandardTesting::_GetMinLocation(const int * pSumLineWidth,int iArrLength)
{
    int iMinLocateM = 0;
 ...
 * @return
 */
int ThreadStandardTesting::_GetTLineLoction(const int * pDataArr, int iOrgCenterxLocation)
{
 ...
 * @return 面积积分
 */
int ThreadStandardTesting::_ImageAnalysisProcess(int * pDataArr, int iOrgLineCenterX, int iPicWide)
{
#if 0
...
int ThreadStandardTesting::_FindFirstItem(QString strPath, ENUM_LOCATION_TYPE type)
{
    QImage qTarImg;
                   ...
                   QVector<double> T8Value)
{
    ...
    AvgC8 = dIntensityC8 / dRatioC6Vector.count();
